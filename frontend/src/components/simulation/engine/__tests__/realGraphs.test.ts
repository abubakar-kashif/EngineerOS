...
describe("real measurement graphs", () => {
  it("Ohm's Law, divider, KCL, and KVL use the known 12 V / 1 kΩ / 4 kΩ solve", () => {
    ...
    expect(kcl.metadata?.labels).toEqual(["I_R1", "I_R2", "ΣI"]);
    expect(kcl.metadata?.labels).not.toContain("I_R3");
    ...
    expect(kvl.metadata?.labels).toEqual(["Vs", "V_R1", "V_R2", "ΣV"]);
  ...
  it("does not invent a third branch current when only two exist", () => {
    ...
    expect(labels.filter((l) => l.startsWith("I_") && l !== "ΣI")).toHaveLength(2);
  });

  it("KCL labels use designators for three physical branches", () => {
    const graphs = solveCircuit(seriesParallel12V()).graphs!;
    const labels = getGraphById(graphs, "current_signals")?.metadata?.labels as string[];
    expect(labels).toEqual(["I_R1", "I_R2", "I_R3", "ΣI"]);
  });
